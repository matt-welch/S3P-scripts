...
# unless the function is being called by bash_colors.sh, 
# assume it's being source'd by another script
# no need to do anything for bash_colors.sh
if [ "$0" == *"bash_colors.sh"* ] ; then 
    echo "Parent: $0"
    # otherwise, 
    if [ -z "$1" ] ; then 
